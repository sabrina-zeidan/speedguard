...
class SpeedGuardWidgets
{
    function __construct()
    {
        ...
        if (!empty($options)) {
            if ($options['show_dashboard_widget'] === 'on') {
                add_action('wp_'.(defined('SPEEDGUARD_MU_NETWORK') ? 'network_' : ''). 'dashboard_setup', array( $this,'speedguard_dashboard_widget'));
            }
            if (($options['show_ab_widget'] === 'on') && !is_admin()) {
                add_action('admin_bar_menu', array( $this,'speedguard_admin_bar_widget'), 710);
            }
        }
    }
    ...
    public static function speedguard_dashboard_widget_function($post = '', $args = '')
    {
        ...
        if (!empty($average_load_time)) {
            ...
            $content = sprintf(
                __('First %1$sadd URLs%2$s that should be guarded.', 'speedguard'),
    ...
    public static function about_meta_box()
    {
        ...
        $rate_it = sprintf(
            __('If you like it, I would greatly appreciate if you add your %1$s★★★★★%2$s to spread the love.', 'speedguard'),
            '<a class="rate-link" href="' .$rate_link. '" target="_blank">',
            '</a>'
        );
        $translate_link = 'https://translate.wordpress.org/projects/wp-plugins/speedguard/';
        $translate_it = sprintf(
            __('You can also help to %1$stranslate it to your language%2$s so that more people will be able to use it ❤︎', 'speedguard'),
            '<a href="' .$translate_link. '" target="_blank">',
            '</a>'
        );
