...
class SpeedGuard_List_Table extends WP_List_Table
{
    ...
    {
        return sprintf(
            '<input type="checkbox" name="guarded-pages[]" value="%s" />',
            $item['guarded_page_id']
        );
    }
    //Columns names
                ...
                $code = $url_to_add.'|'.$connection;
                    ...
                    //TODO always pass blog id
                        if (!empty($guarded_post_blog_id)) {
                            $update_field = update_post_meta($target_page_id, 'guarded_post_blog_id', $guarded_post_blog_id);
                        }
